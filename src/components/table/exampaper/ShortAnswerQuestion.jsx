...
const ShortAnswerQuestion = React.createClass({
    ...
    handleUpdateQuestion(type, value){
        updateQuestion({
            qid: this.props.questionInfo.get('id'),
            examination: type == 'title' ? value : this.props.questionInfo.get('examination'),
            comment: this.props.questionInfo.get('comment'),
            description: this.props.questionInfo.get('description'),
            difficulty: this.props.questionInfo.get('difficulty'),
            kind: this.props.questionInfo.get('kind'),
            drawZone: type == 'title' ? this.props.questionInfo.get('drawZone') : value,
            score: this.props.questionInfo.get('score'),
        })
            ...
            <div className={styles.question} onClick={(e) => {
            ...
            }}>
                {
                    this.state.editingQuestion ? <Ueditor name={this.props.questionInfo.get('id')}
                                                          initialContent={this.props.questionInfo.get('examination') || '请输入题目'}
                                                          onDestory={this.handleUpdateQuestion.bind(this, 'title')}
                                                          initialHeight={QUESTION_HEIGHT}/>
                        : <span
                            dangerouslySetInnerHTML={{__html: addHttpPrefix(this.props.questionInfo.get('examination')) || '请输入题目'}}></span>
                }
                ...
                if (record.key > -1) {
                    ...
                    return (
                        <div className={styles.question}>
                            {
                                this.state.editingAnswerItem ?
                                    <Ueditor name={this.props.questionInfo.getIn(['optionPojoList', record.key, 'id'])}
                                             initialContent={text || '输入选项内容'}
                                             onDestory={this.handleUpdateOption.bind(this)}
                                             initialHeight={OPTION_HEIGHT}/> :
                                    <span dangerouslySetInnerHTML={{__html: addHttpPrefix(text) || '输入选项内容'}}></span>
                            }
                        </div>
                    )
                } else {
                    ...
                    return (
                        <div className={styles.question}>
                            {
                                this.state.editingDrawZone ?
                                    <Ueditor name={this.props.questionInfo.get('id') + 'drawZone'}
                                             onDestory={this.handleUpdateQuestion.bind(this, 'drawZone')}
                                             initialHeight={QUESTION_HEIGHT}/> : <span >{text || '作图区'}</span>
                            }
                        </div>
                    )
                }
    ...
    handlerSetHardness(value){
        updateQuestion({
            ...
            difficulty: value,
            kind: this.props.questionInfo.get('kind'),
            drawZone: this.props.questionInfo.get('drawZone'),
            score: this.props.questionInfo.get('score'),
        });
