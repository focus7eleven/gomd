        ...
        const tableHeader = [{
            ...
            render: (text, record) => {
                ...
                return this.props.type == 'draft' ? (<div style={{width: '200px'}}>
                        <Button type="primary" onClick={this.handleEditPaper.bind(this, text.id)}
                                style={{marginRight: '10px'}}>编辑</Button>
                        <Button onClick={this.handleDeletePaper.bind(this, text.id, 1)}
                                className={styles.deleteButton}>删除</Button>

                    </div>) : (<div style={{width: '200px'}}>
                        <Button type="primary" onClick={this.handleWatchPaper.bind(this, text.id)}
                                style={{marginRight: '10px'}}>查看</Button>
                        {record.count==0?<Button onClick={this.handleDeletePaper.bind(this,text.id, 0)}
                                                 className={styles.deleteButton}>删除</Button>:null}
                    </div>)
            }
        }]
            ...
            <div className={styles.container}>
                <div className={styles.header}>
                    <Button type='primary' onClick={() => {
                        this.context.router.push(`/index/question-exampaper/newexampaper`)
                    }}>新建试卷</Button><ExampaperFilter />
                </div>
                <div className={styles.body}>
                    <TableComponent dataType="exampaper" tableData={tableData} pageType="selfexampapercenter"
                                    searchStr={this.state.searchStr}></TableComponent>
                </div>
            </div>
        )
    }
