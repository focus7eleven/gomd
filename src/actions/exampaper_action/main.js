...
//删除试卷
export function deletePaper(examId,isDraft) {
        ...
        fetch(config.api.exampaper.deletePaper, {
            ...
            if (res.title == 'Success') {
                dispatch({
                    types: GET_TABLEDATA,
                    callAPI: () => {
                        return fetch(config.api.exampaper.getTableData('showExamList.json', '', 1, '', '', isDraft), {
                            method: 'GET',
                            headers: {
                                'from': 'nodejs',
                                'token': sessionStorage.getItem('accessToken'),
                            ...
                            return {mainData: res, otherMsg: {search: '', subjectId: '', gradeId: ''}}
                        })
                    }
                })
            } else {
    ...
    return {
        ...
        callAPI: () => {
            return fetch(config.api.exampaper.getTableData(type, search, currentPage, subjectId, gradeId, draft), {
                ...
                mainData: res,
                otherMsg: {search, currentPage, phaseCode, subjectId, termId}
            }))
        },
    }
}
